import { emailClientTemplate } from '@/modules/lanceandshamy/email/client'; //change client name
import { emailGuestTemplate } from '@/modules/lanceandshamy/email/guest'; //change client name
import { RSVPDetailsType } from '@/modules/lanceandshamy/reducer/RSVPInit'; //change client name
...
export async function POST(request: Request) {
  ...
  const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: process.env.LS_CLIENTID, //change vercel link and .env
      pass: process.env.LS_APIKEY //change vercel link and .env
    }
  });

  const clientname = 'lanceandshamy';//change client name
  const hostname = {
  ...
  const SendSheet = async () => {
    try {
      const response = await fetch("https://sheetdb.io/api/v1/hru8zcm05e5lp", { //change sheetdb link
        method: 'POST',
        headers: {
