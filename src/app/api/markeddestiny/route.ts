import { emailClientTemplate } from '@/modules/markeddestiny/email/client'; //change client name
import { emailGuestTemplate } from '@/modules/markeddestiny/email/guest'; //change client name
import { RSVPDetailsType } from '@/modules/markeddestiny/reducer/RSVPInit'; //change client name
...
export async function POST(request: Request) {
  ...
  const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: process.env.MM_CLIENTID, //change vercel link and .env
      pass: process.env.MM_APIKEY //change vercel link and .env
    }
  });

  const clientname = 'markeddestiny';//change client name
  const hostname = {
  ...
  const emailerName = {
    name: 'Mark and Khristine Wedding <markandkhristinewedding@gmail.com', //google account 
  ...
  const SendSheet = async () => {
    try {
      const response = await fetch("https://sheetdb.io/api/v1/z23m21w6e4ukj", { //change sheetdb link
        method: 'POST',
        headers: {
