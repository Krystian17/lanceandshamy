import { emailClientTemplate } from "@/modules/markandlydgie/email/client";
import { emailGuestTemplate } from "@/modules/markandlydgie/email/guest";
import { RSVPDetailsType } from "@/modules/markandlydgie/reducer/RSVPInit";
...
export async function POST(request: Request) {
  ...
  const transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: process.env.ML_CLIENTID,
      pass: process.env.ML_APIKEY,
    },
  });

  const clientname = "markandlydgie";
  const hostname = {
  ...
  const emailerName = {
    name: "Mark and Lydgie Wedding <markandlydgiewedding@gmail.com>",
    address: "lydgielen@gmail.com",
  ...
  const SendSheet = () =>
    fetch("https://sheetdb.io/api/v1/m0an6ns14ilcu", {
      method: "POST",
      headers: {
