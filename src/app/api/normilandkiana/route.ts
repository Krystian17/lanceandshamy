import { emailClientTemplate } from "@/modules/normilandkiana/email/client";
import { emailGuestTemplate } from "@/modules/normilandkiana/email/guest";
import { RSVPDetailsType } from "@/modules/normilandkiana/reducer/RSVPInit";
...
export async function POST(request: Request) {
  ...
  const transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: process.env.NK_CLIENTID,
      pass: process.env.NK_APIKEY,
    },
  });

  const clientname = "normilandkiana";
  const hostname = {
  ...
  const emailerName = {
    name: "Normil and Kiana Wedding <normilandkianawedding@gmail.com>",
    address: "raquedansomido.wedding@gmail.com",
  ...
  const SendSheet = () =>
    fetch("https://sheetdb.io/api/v1/awsbhevdbk3fx", {
      method: "POST",
      headers: {
