import { emailClientTemplate } from "@/modules/markandkata/email/client";
import { emailGuestTemplate } from "@/modules/markandkata/email/guest";
import { RSVPDetailsType } from "@/modules/markandkata/reducer/RSVPInit";
...
export async function POST(request: Request) {
  ...
  const transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: process.env.MK_CLIENTID,
      pass: process.env.MK_APIKEY,
    },
  });

  const clientname = "markandkata";
  const hostname = {
  ...
  const emailerName = {
    name: "Mark and Kata Wedding <markandkatawedding@gmail.com>",
    address: "klpl.lainez@gmail.com",
  ...
  const SendSheet = () =>
    fetch("https://sheetdb.io/api/v1/zgjdambqovj8k", {
      method: "POST",
      headers: {
