import { emailClientTemplate } from '@/modules/arjayandcamille/email/client'; //change client name
import { emailGuestTemplate } from '@/modules/arjayandcamille/email/guest'; //change client name
import { RSVPDetailsType } from '@/modules/arjayandcamille/reducer/RSVPInit'; //change client name
...
export async function POST(request: Request) {
  ...
  const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: process.env.AC_CLIENTID, //change vercel link and .env
      pass: process.env.AC_APIKEY //change vercel link and .env
    }
  });

  const clientname = 'arjayandcamille';//change client name
  const hostname = {
  ...
  const emailerName = {
    name: 'Arjay and Camille Wedding <arjayandcamille@gmail.com', //google account 
  ...
  const SendSheet = async () => {
    try {
      const response = await fetch("https://sheetdb.io/api/v1/hqkcotcz880kl", { //change sheetdb link
        method: 'POST',
        headers: {
